...
def file2df(path):
    ...
    """
    df = pd.read_csv(path, header=None, names=['AllEducation'])
...
ed_path = '/notebooks/Inputs/AllEducation.txt'  
...
# get a list of indicies of the df which are higher educations
idx = []
for he in higher_ed:
...
for ad, folder in job_ads.items():
    ...
    output_RelOtherEd_path = f"/notebooks/Resume Components/{folder}/Rel_OtherEd.txt"  
    ...
    # write to relevant Job ad folder in Resume Components folder
    output_HigherEd_path = f"/notebooks/Resume Components/{folder}/HigherEd.txt"  
