...
wh_path = "/notebooks/Inputs/WorkHistory_Achievments.csv"
...
for ad, folder in job_ads.items():
    ...
    ## Job ad ##
    # path to the job ad
    job_ad_path = f'/notebooks/Job Ads/New Ad/{ad}'  
    ...
    output_path =   f'/notebooks/Resume Components/{folder}/sorted_achievements.csv'
...
wh_grouped = wh_sorted_df.groupby('role_id').agg({ 'achievement_list' : 'sum'})
 ...
 # Take the role_id, achievement_id and avg_cos_sim, and output to csv
output_path =   f'/notebooks/Resume Components/{folder}/ranked_work_history.csv'
